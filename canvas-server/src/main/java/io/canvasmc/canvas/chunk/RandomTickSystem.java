...
public final class RandomTickSystem {
    ...
    public void tick(RegionizedWorldData worldData) {
        ServerLevel world = worldData.world;
        queue.clear();
        samples.clear();
        weights.clear();

        final BitRandomSource random = worldData.simpleUnsafeLocalRandom;
        final ReferenceList<LevelChunk> entityTickingChunks = worldData.getEntityTickingChunks();
